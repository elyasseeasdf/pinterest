...
const PinCard = ({ pin, onEdit, onDelete, getImageUrl, onSelectExtension, verifiedExtension }) => {
    ...
    return (
        <div className="pin-card animate-fade-in">
            <div className={`pin-image-wrapper ${!verifiedExtension ? 'split-view' : ''}`}>
                {!verifiedExtension ? (
                    <>
                        <div className="split-half png" onClick={() => onSelectExtension(pin.id, '.png')}>
                            <img src={getImageUrl(pin, '.png')} alt="PNG version" loading="lazy" />
                            <div className="extension-badge">PNG</div>
                        </div>
                        <div className="split-half jpg" onClick={() => onSelectExtension(pin.id, '.jpg')}>
                            <img src={getImageUrl(pin, '.jpg')} alt="JPG version" loading="lazy" />
            ...
            <div className="pin-info">
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
                    ...
                    <span style={{ fontSize: '0.7rem', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
                        ...
                        {pin.publishDate ? new Date(pin.publishDate).toLocaleDateString() : 'No date'}
                    </span>
                </div>

                <h3 className="pin-title" style={{ fontSize: '1.1rem', marginBottom: '1.25rem', minHeight: '2.8rem' }}>{pin.title}</h3>

                <div className="card-actions" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem' }}>
                    <button className="action-btn" onClick={() => onEdit(pin)} title="Edit Pin">
                        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                    ...
                    <button className="action-btn delete" onClick={() => onDelete(pin.id)}>
                        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
